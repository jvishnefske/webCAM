...
pub enum PeripheralRequirement {
    Adc { logical_channel: u8 },
    Pwm { logical_channel: u8 },
    GpioOutput { logical_pin: u8 },
    GpioInput { logical_pin: u8 },
    Uart { logical_port: u8, direction: UartDirection },
    Encoder { logical_channel: u8 },
    I2c { logical_bus: u8, address: u8 },
    Stepper { logical_port: u8 },
    StallGuard { logical_port: u8, address: u8 },
}
        ...
        if !has_assignment_for(&entry.requirement, &config.assignments) {
            errors.push(ConfigError {
                ...
                message: alloc::format!(
                    ...
                    entry.block_name, entry.block_id, entry.requirement
                ),
    ...
    for i in 0..all_pins.len() {
        for j in (i + 1)..all_pins.len() {
            if all_pins[i] == all_pins[j] {
                errors.push(ConfigError {
                    severity: ConfigSeverity::Error,
                    message: alloc::format!("pin '{}' is assigned to multiple peripherals", all_pins[i]),
                    block_id: None,
                });
            }
        }
...
fn matches_requirement(assignment: &PinAssignment, req: &PeripheralRequirement) -> bool {
    match (assignment, req) {
        (PinAssignment::Adc { logical_channel: a, .. }, PeripheralRequirement::Adc { logical_channel: r }) => a == r,
        (PinAssignment::Pwm { logical_channel: a, .. }, PeripheralRequirement::Pwm { logical_channel: r }) => a == r,
        (PinAssignment::Gpio { logical_pin: a, direction: GpioDirection::Output, .. }, PeripheralRequirement::GpioOutput { logical_pin: r }) => a == r,
        (PinAssignment::Gpio { logical_pin: a, direction: GpioDirection::Input, .. }, PeripheralRequirement::GpioInput { logical_pin: r }) => a == r,
        (PinAssignment::Uart { logical_port: a, .. }, PeripheralRequirement::Uart { logical_port: r, .. }) => a == r,
        (PinAssignment::Encoder { logical_channel: a, .. }, PeripheralRequirement::Encoder { logical_channel: r }) => a == r,
        (PinAssignment::I2c { logical_bus: a, .. }, PeripheralRequirement::I2c { logical_bus: r, .. }) => a == r,
        (PinAssignment::Stepper { logical_port: a, .. }, PeripheralRequirement::Stepper { logical_port: r }) => a == r,
        (PinAssignment::Stepper { logical_port: a, .. }, PeripheralRequirement::StallGuard { logical_port: r, .. }) => a == r,
        _ => false,
    }
}
...
fn collect_all_pins(assignments: &[PinAssignment]) -> Vec<&str> {
    ...
    for a in assignments {
        match a {
            ...
            PinAssignment::I2c { sda_pin, scl_pin, .. } => {
                pins.push(sda_pin.as_str());
                pins.push(scl_pin.as_str());
            }
            PinAssignment::Stepper { step_pin, dir_pin, enable_pin, .. } => {
...
fn validate_assignment_against_capabilities(
    ...
    match assignment {
        ...
        PinAssignment::Uart { peripheral, .. } => {
            if !capabilities.uart_peripherals.iter().any(|c| c.peripheral == *peripheral) {
                errors.push(ConfigError {
                    severity: ConfigSeverity::Error,
                    message: alloc::format!("UART '{}' is not available on this target", peripheral),
                    block_id: None,
                });
            }
        }
        PinAssignment::Encoder { timer, .. } => {
            if !capabilities.encoder_pins.iter().any(|c| c.timer == *timer) {
                errors.push(ConfigError {
                    severity: ConfigSeverity::Error,
                    message: alloc::format!("encoder timer '{}' is not available on this target", timer),
                    block_id: None,
                });
            }
        }
        PinAssignment::I2c { peripheral, .. } => {
            if !capabilities.i2c_peripherals.iter().any(|c| c.peripheral == *peripheral) {
                errors.push(ConfigError {
                    severity: ConfigSeverity::Error,
...
pub fn extract_requirements(
    blocks: &[(u32, &str, &str, &serde_json::Value)],
) -> RequirementSet {
        ...
        match block_type {
            "adc_source" => {
                ...
                requirements.push(RequirementEntry {
                    ...
                    requirement: PeripheralRequirement::Adc { logical_channel: ch },
                });
            }
            "pwm_sink" => {
                let ch = config.get("channel").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
                    requirement: PeripheralRequirement::Pwm { logical_channel: ch },
                });
            }
            "gpio_out" => {
            ...
            "uart_tx" => {
                ...
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
                    requirement: PeripheralRequirement::Uart { logical_port: port, direction: UartDirection::Tx },
                });
            }
            "uart_rx" => {
                let port = config.get("port").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
                    requirement: PeripheralRequirement::Uart { logical_port: port, direction: UartDirection::Rx },
                });
            }
            "encoder" => {
                let ch = config.get("channel").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
                    requirement: PeripheralRequirement::Encoder { logical_channel: ch },
                });
            }
            "ssd1306_display" => {
                let bus = config.get("i2c_bus").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                let addr = config.get("address").and_then(|v| v.as_u64()).unwrap_or(0x3C) as u8;
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
                    requirement: PeripheralRequirement::I2c { logical_bus: bus, address: addr },
                });
            }
            "tmc2209_stepper" => {
                let port = config.get("uart_port").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
            ...
            "tmc2209_stallguard" => {
                let port = config.get("uart_port").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                let addr = config.get("uart_addr").and_then(|v| v.as_u64()).unwrap_or(0) as u8;
                requirements.push(RequirementEntry {
                    block_id,
                    block_name: String::from(block_name),
                    requirement: PeripheralRequirement::StallGuard { logical_port: port, address: addr },
                });
            }
...
fn rp2040_capabilities() -> TargetCapabilities {
    TargetCapabilities {
        ...
        adc_pins: alloc::vec![
            AdcCapability { pin: String::from("GP26"), peripheral: String::from("ADC"), hw_channel: 0 },
            AdcCapability { pin: String::from("GP27"), peripheral: String::from("ADC"), hw_channel: 1 },
            AdcCapability { pin: String::from("GP28"), peripheral: String::from("ADC"), hw_channel: 2 },
            AdcCapability { pin: String::from("GP29"), peripheral: String::from("ADC"), hw_channel: 3 },
        ],
        pwm_pins: (0..16).map(|i| {
            let slice = i / 2;
        ...
        gpio_pins: (0..30).map(|i| GpioCapability {
            pin: alloc::format!("GP{i}"),
            can_input: true,
            can_output: true,
            has_pull: true,
        ...
        uart_peripherals: alloc::vec![
            UartCapability {
                ...
                tx_pins: alloc::vec![String::from("GP0"), String::from("GP12"), String::from("GP16")],
                rx_pins: alloc::vec![String::from("GP1"), String::from("GP13"), String::from("GP17")],
            },
            UartCapability {
        ...
        i2c_peripherals: alloc::vec![
            I2cCapability {
                ...
                sda_pins: alloc::vec![String::from("GP0"), String::from("GP4"), String::from("GP8"), String::from("GP12"), String::from("GP16"), String::from("GP20")],
                scl_pins: alloc::vec![String::from("GP1"), String::from("GP5"), String::from("GP9"), String::from("GP13"), String::from("GP17"), String::from("GP21")],
            },
            I2cCapability {
                peripheral: String::from("I2C1"),
                sda_pins: alloc::vec![String::from("GP2"), String::from("GP6"), String::from("GP10"), String::from("GP14"), String::from("GP18"), String::from("GP26")],
                scl_pins: alloc::vec![String::from("GP3"), String::from("GP7"), String::from("GP11"), String::from("GP15"), String::from("GP19"), String::from("GP27")],
            },
        ],
        stepper_slots: Vec::new(),
    }
...
fn stm32f4_capabilities() -> TargetCapabilities {
    TargetCapabilities {
        ...
        adc_pins: alloc::vec![
            AdcCapability { pin: String::from("PA0"), peripheral: String::from("ADC1"), hw_channel: 0 },
            AdcCapability { pin: String::from("PA1"), peripheral: String::from("ADC1"), hw_channel: 1 },
            AdcCapability { pin: String::from("PA4"), peripheral: String::from("ADC1"), hw_channel: 4 },
            AdcCapability { pin: String::from("PA5"), peripheral: String::from("ADC1"), hw_channel: 5 },
            AdcCapability { pin: String::from("PA6"), peripheral: String::from("ADC1"), hw_channel: 6 },
            AdcCapability { pin: String::from("PA7"), peripheral: String::from("ADC1"), hw_channel: 7 },
            AdcCapability { pin: String::from("PB0"), peripheral: String::from("ADC1"), hw_channel: 8 },
            AdcCapability { pin: String::from("PB1"), peripheral: String::from("ADC1"), hw_channel: 9 },
        ],
        pwm_pins: alloc::vec![
            PwmCapability { pin: String::from("PA8"), timer: String::from("TIM1"), channel: String::from("1") },
            PwmCapability { pin: String::from("PA9"), timer: String::from("TIM1"), channel: String::from("2") },
            PwmCapability { pin: String::from("PA10"), timer: String::from("TIM1"), channel: String::from("3") },
            PwmCapability { pin: String::from("PA11"), timer: String::from("TIM1"), channel: String::from("4") },
            PwmCapability { pin: String::from("PA0"), timer: String::from("TIM2"), channel: String::from("1") },
            PwmCapability { pin: String::from("PA1"), timer: String::from("TIM2"), channel: String::from("2") },
        ],
        gpio_pins: {
            let mut pins = Vec::new();
            for port in ['A', 'B', 'C'] {
                let count = match port { 'A' => 16, 'B' => 16, _ => 14 };
                for i in 0..count {
                    pins.push(GpioCapability {
                        pin: alloc::format!("P{port}{i}"),
                        can_input: true, can_output: true, has_pull: true,
                    });
                }
            }
            pins
        ...
        encoder_pins: alloc::vec![
            EncoderCapability { timer: String::from("TIM2"), pin_a: String::from("PA0"), pin_b: String::from("PA1") },
            EncoderCapability { timer: String::from("TIM3"), pin_a: String::from("PA6"), pin_b: String::from("PA7") },
        ],
        i2c_peripherals: alloc::vec![
            I2cCapability {
                peripheral: String::from("I2C1"),
...
fn esp32c3_capabilities() -> TargetCapabilities {
    TargetCapabilities {
        ...
        adc_pins: alloc::vec![
            AdcCapability { pin: String::from("GPIO0"), peripheral: String::from("ADC1"), hw_channel: 0 },
            AdcCapability { pin: String::from("GPIO1"), peripheral: String::from("ADC1"), hw_channel: 1 },
            AdcCapability { pin: String::from("GPIO2"), peripheral: String::from("ADC1"), hw_channel: 2 },
            AdcCapability { pin: String::from("GPIO3"), peripheral: String::from("ADC1"), hw_channel: 3 },
            AdcCapability { pin: String::from("GPIO4"), peripheral: String::from("ADC1"), hw_channel: 4 },
        ],
        pwm_pins: (0..6).map(|i| PwmCapability {
            pin: alloc::format!("GPIO{i}"),
            timer: String::from("LEDC"),
            channel: alloc::format!("{i}"),
        }).collect(),
        gpio_pins: (0..22).map(|i| GpioCapability {
            pin: alloc::format!("GPIO{i}"),
            can_input: true,
            can_output: true,
            has_pull: true,
        ...
        i2c_peripherals: alloc::vec![
            I2cCapability {
                peripheral: String::from("I2C0"),
                sda_pins: alloc::vec![String::from("GPIO1"), String::from("GPIO3"), String::from("GPIO5")],
                scl_pins: alloc::vec![String::from("GPIO0"), String::from("GPIO2"), String::from("GPIO4")],
            },
        ],
        stepper_slots: Vec::new(),
    }
...
fn stm32g0b1_capabilities() -> TargetCapabilities {
    TargetCapabilities {
        ...
        adc_pins: alloc::vec![
            AdcCapability { pin: String::from("PA0"), peripheral: String::from("ADC1"), hw_channel: 0 },
            AdcCapability { pin: String::from("PA1"), peripheral: String::from("ADC1"), hw_channel: 1 },
            AdcCapability { pin: String::from("PA2"), peripheral: String::from("ADC1"), hw_channel: 2 },
            AdcCapability { pin: String::from("PA3"), peripheral: String::from("ADC1"), hw_channel: 3 },
            AdcCapability { pin: String::from("PA4"), peripheral: String::from("ADC1"), hw_channel: 4 },
            AdcCapability { pin: String::from("PA5"), peripheral: String::from("ADC1"), hw_channel: 5 },
            AdcCapability { pin: String::from("PA6"), peripheral: String::from("ADC1"), hw_channel: 6 },
            AdcCapability { pin: String::from("PA7"), peripheral: String::from("ADC1"), hw_channel: 7 },
        ...
        gpio_pins: {
            let mut pins = Vec::new();
            for port in ['A', 'B', 'C', 'D'] {
                let count = match port { 'A' => 16, 'B' => 16, 'C' => 16, _ => 16 };
                for i in 0..count {
                    pins.push(GpioCapability {
                        pin: alloc::format!("P{port}{i}"),
                        can_input: true, can_output: true, has_pull: true,
        ...
        uart_peripherals: alloc::vec![
            ...
            UartCapability {
                ...
                tx_pins: alloc::vec![String::from("PB8"), String::from("PB10"), String::from("PC4")],
                rx_pins: alloc::vec![String::from("PB9"), String::from("PB11"), String::from("PC5")],
            },
            UartCapability {
        ...
        i2c_peripherals: alloc::vec![
            ...
            I2cCapability {
                ...
                sda_pins: alloc::vec![String::from("PA12"), String::from("PB11"), String::from("PB14")],
                scl_pins: alloc::vec![String::from("PA11"), String::from("PB10"), String::from("PB13")],
            },
        ],
        stepper_slots: alloc::vec![
            StepperCapability {
                label: String::from("X"),
...
mod tests {
    ...
    fn validate_pin_conflict() {
        ...
        let result = validate_config(&reqs, &caps, &config);
        assert!(result.is_err());
        assert!(result.unwrap_err().iter().any(|e| e.message.contains("multiple peripherals")));
    }

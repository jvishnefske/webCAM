//! Built-in block implementations for the RustCAM dataflow engine.
//!
...
// Re-export BlockRegistry and BlockTypeInfo from module-traits
pub use module_traits::{BlockRegistry, BlockTypeInfo as TraitBlockTypeInfo};
...
/// Built-in block registry implementing [`BlockRegistry`].
pub struct BuiltinBlocks;

impl BlockRegistry for BuiltinBlocks {
    fn block_types(&self) -> Vec<TraitBlockTypeInfo> {
        available_block_types()
            .into_iter()
            .map(|b| TraitBlockTypeInfo {
                block_type: b.block_type.to_string(),
                display_name: b.name.to_string(),
...
mod tests {
    ...
    // ── BlockRegistry trait tests ──────────────────────

    #[test]
    fn builtin_blocks_implements_registry_trait() {
        let registry = BuiltinBlocks;
        let types = registry.block_types();
        assert!(!types.is_empty());
        // Verify some known block types exist
        let ids: Vec<&str> = types.iter().map(|t| t.block_type.as_str()).collect();
        assert!(ids.contains(&"constant"), "missing constant");
        assert!(ids.contains(&"add"), "missing add");
        assert!(ids.contains(&"state_machine"), "missing state_machine");
    }

    #[test]
    fn create_all_block_types() {
        let registry = BuiltinBlocks;
        let types = registry.block_types();
        for bt in &types {
            // Use a default config that works for most block types
            let config = match bt.block_type.as_str() {
                "constant" => r#"{"value": 0.0}"#,
                "gain" => r#"{"gain": 1.0}"#,
                "clamp" => r#"{"min": 0.0, "max": 1.0}"#,
                "pubsub_sink" | "pubsub_source" => r#"{"topic":"t","port_kind":"Float"}"#,
                "state_machine" => r#"{"states":["idle"],"initial":"idle"}"#,
                "udp_source" | "udp_sink" => r#"{"address":"127.0.0.1:5000"}"#,
                "adc_source" => r#"{"channel":0,"resolution_bits":12}"#,
                "pwm_sink" => r#"{"channel":0,"frequency_hz":1000}"#,
                "gpio_out" => r#"{"pin":13}"#,
                "gpio_in" => r#"{"pin":2}"#,
                ...
                "register" => r#"{"initial_value": 0.0}"#,
                _ => "{}",
            };
            let result = registry.create(&bt.block_type, config);
            assert!(
                result.is_ok(),

...
mod tests {
    ...
    fn test_disconnect_nonexistent_returns_false() {
        let engine = GraphEngine::new();
        assert!(!engine.clone_for_test().disconnect(999));
    }
    ...
    // Helper — the engine is not Clone, so use this in tests that need it.
    impl GraphEngine {
        fn clone_for_test(&self) -> GraphEngine {
            let mut e = GraphEngine::new();
            e.restore(&self.snapshot());
            e
        }
    }
}

...
/// An Image Definition with a vector table pointer (3 item words).
pub type ImageDefVt = Block<3>;
...
impl ImageDefVt {
    /// Construct an IMAGE_DEF with image type and vector table pointer.
    pub const fn secure_exe_vt(vtor: u32) -> Self {
        let [vt0, vt1] = item_vector_table(vtor);
        Self::new([item_image_type_exe(Security::Secure, Architecture::Arm), vt0, vt1])
    }

    /// Construct a non-secure IMAGE_DEF with vector table pointer.
    pub const fn non_secure_exe_vt(vtor: u32) -> Self {
        let [vt0, vt1] = item_vector_table(vtor);
        Self::new([item_image_type_exe(Security::NonSecure, Architecture::Arm), vt0, vt1])
    }
}

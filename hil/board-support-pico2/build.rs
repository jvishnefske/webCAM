...
fn main() {
    ...
    compress_combined_frontend(out);
}
...
fn compress_combined_frontend(out: &PathBuf) {
    println!("cargo::rustc-check-cfg=cfg(has_combined_frontend)");

    let manifest_dir = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
    let dist = manifest_dir.join("../combined-frontend/dist");
    println!("cargo:rerun-if-changed={}", dist.display());

    if !dist.is_dir() {
        println!("cargo:warning=No combined-frontend/dist/ found, skipping");
        return;
    }
    ...
    for entry in entries.flatten() {
        ...
        *found_flag = true;
        ...
        println!(
            "cargo:warning=Combined frontend: {name} {} -> {} bytes ({:.0}%)",
            data.len(),
            compressed.len(),
            (compressed.len() as f64 / data.len() as f64) * 100.0,
        );
    ...
    if found_html && found_js && found_wasm && found_css {
        println!("cargo:rustc-cfg=has_combined_frontend");
    } else {
        println!("cargo:warning=Combined frontend dist/ incomplete, skipping");
    }
}
